			...
			<div id="employeeUpdate">
				...
				<form action="<%=request.getContextPath()%>/employeeUpdate"
					...
					<form action="<%=request.getContextPath()%>/employeeRegister"
						...
						if (cmd.equals("update"))
						%>
						<a href="<%=request.getContextPath()%>/detailEmployee?userId=<%= userId %>"> <input
						...
						%>
												<a href="<%=request.getContextPath()%>/view/employeeRegister.jsp?cmd=update?userId=<%= userId %>"> <input
							type="submit" value="キャンセル"
							style="width: 120px; height: 50px; font-size: large;">
						</a>
						<%
