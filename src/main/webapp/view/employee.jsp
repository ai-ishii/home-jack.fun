<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>社員紹介</title>
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/css/style.css">
</head>
<body>

	<%@include file="../common/header.jsp"%>

	<div id="employee">
		<!-- タイトル部分 -->
		<table style="width: 100%;">
			<tr>
				<td style="width: 30%;">&nbsp;</td>
				<td style="width: 40%;">
					<h1>社員紹介</h1>
				</td>
				<td style="width: 30%;"><input type="submit" value="登録"
					style="width: 80px; height: 50px; font-size: large;"></td>
			</tr>
		</table>

		<div id="employee_list">

			<!-- 社員一覧の1行分（5枚ずつ） -->
			<div id="employee_line">
				<a href="detailEmployee.jsp">
					<div id="employee_card">
						<img src="<%=request.getContextPath()%>/img/photo1.png" alt="社員画像">
						<p id="employee_name">月光 名</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a> <a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo2.png" alt="社員画像">
						<p id="employee_name">山田 太郎</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a> <a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo3.png" alt="社員画像">
						<p id="employee_name">鈴木 花子</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a> <a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo4.png" alt="社員画像">
						<p id="employee_name">姓 名</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a> <a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo5.png" alt="社員画像">
						<p id="employee_name">姓 名</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a>
			</div>

			<!-- 社員一覧の1行分 -->
			<div id="employee_line">
				<a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo6.png" alt="社員画像">
						<p id="employee_name">姓 名</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a> <a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo6.png" alt="社員画像">
						<p id="employee_name">姓 名</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a> <a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo6.png" alt="社員画像">
						<p id="employee_name">姓 名</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a> <a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo6.png" alt="社員画像">
						<p id="employee_name">姓 名</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a> <a href="detailEmployee.jsp">
					<div id="employee_card">
						<img id="employee_img"
							src="<%=request.getContextPath()%>/img/photo6.png" alt="社員画像">
						<p id="employee_name">姓 名</p>
						<p id="employee_detail">第1事業部 第1グループ</p>
						<p id="employee_detail">2025年4月入社</p>
					</div>
				</a>
			</div>
		</div>
	</div>
	
</body>
</html>
