...
public class LicenseConfirmServlet extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Long oldReceiptId = request.getParameter("receiptFileId") != null
				&& !request.getParameter("receiptFileId").isEmpty()
						? Long.parseLong(request.getParameter("receiptFileId"))
						: null;
		Long oldPassingId = request.getParameter("passingFileId") != null
				&& !request.getParameter("passingFileId").isEmpty()
						? Long.parseLong(request.getParameter("passingFileId"))
						: null;
		...
		if (receiptPart != null && receiptPart.getSize() > 0) {
			...
			//  新規の申請の処理
				...
				newReceiptId = requestDAO.insertReceiptFileProvisional(receiptBytes,
						receiptPart.getSubmittedFileName());
			}
		...
		if (passingPart != null && passingPart.getSize() > 0) {
			...
			//  新規の申請の処理    
				...
				newPassingId = requestDAO.insertPassingFileProvisional(passingBytes,
						passingPart.getSubmittedFileName());
			}
		...
		// ユーザー入力ファイルに receiptBytes があれば、それを formValues にコピー
		if (userInputFile.containsKey("receiptBytes"))
			formValues.put("receiptBytes", userInputFile.get("receiptBytes"));
		if (userInputFile.containsKey("passingBytes"))
			formValues.put("passingBytes", userInputFile.get("passingBytes"));

		// licenseRequestExclusiveにセット
		...
		//  親テーブルにapplicantIdを登録して申請IDを発行
		try {
			requestId = requestDAO.insertLicenseRequestID(applicantId);
		} catch (Exception e) {
			e.printStackTrace();
			request.setAttribute("errorMessage", "申請ID生成に失敗");
			request.getRequestDispatcher("/view/licenseForm.jsp").forward(request, response);
			return;
		}
